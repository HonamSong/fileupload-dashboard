...
로그인 → 파일 선택 → 공유 링크(비밀번호·만료) 생성 → 공개 다운로드 페이지 흐름:
...
### 공유 링크 (계정 없는 사용자에게 공유)
- 파일 체크박스로 선택 후 **🔗 공유** → 공개 링크(`/s/<토큰>`) 생성. 계정·API 키 없이 다운로드 가능.
- **비밀번호**(선택, bcrypt) + **만료**(5분 테스트 · 3~12시간 · 1일 기본 · 3/7/14/30일). 만료 시 링크 자동 무효화.
- **여러 파일 선택 시 하나의 zip**으로 묶여 다운로드, 1개면 원본 그대로.
- 링크를 열면 자체 안내 페이지에서 파일명·용량·만료를 보여주고, 비밀번호가 있으면 입력 후 다운로드.
- 공유 다운로드는 접근 로그에 `공유(생성자)`로 기록. 설정 화면(공유 모달)에서 본인/전체(owner·admin) 링크 목록·삭제 관리.
- 생성 권한: editor(owner/admin/user) + 해당 폴더 읽기 권한. 만료된/삭제된 파일의 링크는 자동 정리.
