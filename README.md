...
로그인·역할 기반 권한(RBAC)·폴더별 접근 제어·API 키(서명 검증·만료)·파일 버전 관리·로그(접근/에러/감사)·IP 차단·공유 링크·헬스체크까지 포함합니다.
...
- **체크섬**: 업로드 시 SHA-256 자동 계산·표시. 미리보기: 텍스트/이미지(코드 구문 강조·라인 넘버, view 역할 불가).
- **폴더**: 중첩 폴더 생성/삭제/이동. 각 폴더의 `⋮` 메뉴에서 권한·이동·삭제. 폴더별 파일 개수 표시, 접기.
- **버전 관리**: 같은 폴더에 동일 이름으로 재업로드하면 이전 내용을 버전으로 보관(링크 id 유지).
  - "현재"는 여러 버전 중 하나를 가리키는 포인터 — **되돌리기는 새 버전을 만들지 않고** 현재 표시만 이동.
  - 파일별 "버전" 버튼에서 목록·개별 다운로드·되돌리기·삭제(현재 버전은 삭제 불가).
  - 보관 개수는 설정 > Server에서 지정(기본 10, 0=버전 관리 안 함). 초과 시 오래된 버전부터 자동 정리. 동일 내용 재업로드는 버전 생성 안 함.
- **페이지네이션**: 파일 목록(기본 10, 20/30/50/100), 로그(기본 20, 50/100).
- **리사이즈 레이아웃**: 파일/명령어·미리보기 영역을 드래그로 좌우 조절, 창 높이에 맞춤.
- **탭·상태 유지**: 새로고침해도 마지막으로 보던 탭·하위 탭이 유지됨.
...
- 휴지통은 **삭제자(누가 삭제했는지)** 표시, **체크박스로 여러 파일 일괄 복구/완전삭제**. 폴더 쓰기 권한 없는 항목은 비활성.
- 폴더를 삭제(강제)하면 안의 파일은 **휴지통으로 이동**되어 복구 가능(폴더/하위 폴더는 제거). admin 이상만 비어있지 않은 폴더 삭제 가능.

### API 키
- **개인 키**(사용자당 최대 3개, user 이상) + **서비스 키**(owner/admin 전용, 개수 제한 없음, 소유자 없음).
- 스코프: `download` / `upload` / `all(both)`.
- **만료(임시 키)**: 발급 시 1시간~30일 만료를 지정할 수 있음(0=만료 없음). 만료된 키는 자동 거부·정리되며 개인 키 개수 한도에 포함되지 않음 → 스크립트 결과 업로드용 임시 키에 적합.
- **특정 사용자 명의 발급**: owner/admin은 다른 사용자 명의로 키 발급 가능(개인 키). 서비스 키는 명의 없음.
- **서비스 키 정책**: 발급·관리(비활성/폐기/삭제)는 owner/admin만. user 이상 역할은 목록에서 볼 수 있으나 **키 값은 마스킹**(값은 owner/admin만).
- **HMAC 서명**: 발급되는 키에 서버 비밀키로 서명이 포함되어, 위조/변조 키를 암호학적으로 판별(기존 키는 DB 인증으로 하위 호환).
- **헬스체크**: API KEY 탭에서 키를 골라 `/healthz`를 바로 호출해 상태 확인, curl 명령 복사.

### 로그 (owner/admin, 설정 > 로그)
- **Access Log**: 업로드·다운로드 통합 기록(시각·구분·결과·파일·사용자·API 키·IP·User-Agent) + 거부된 시도(알 수 없는/위조/폐기·비활성·만료 키, 권한 없음)까지. 정렬·검색·필터·페이지네이션. IP에 마우스를 올리면 차단/허용(owner).
- **Error Log**: 시스템 오류(5xx 응답·panic)를 자동 기록(출처·메시지·IP).
- **Audit Log**: 사용자 활동 — 로그인/로그아웃/실패, 파일·폴더·키·사용자·폴더권한·버전의 생성/삭제/변경/이동/되돌리기 등. 그룹 필터·검색.

### 서버 설정 (owner, 설정 > Server — 일반 / IP 접근제어 / 자동 차단 하위 탭)
- **Base URL**: curl 명령어에 표시되는 주소.
- **파일 버전 보관 개수**: 파일당 유지할 버전 수(기본 10, 0=끔).
- **IP 접근 제어**: API 엔드포인트·대시보드 UI 각각 Allow/Block 목록(단일 IP 또는 CIDR). 입력 즉시 형식 검증.
...
|------|:-----:|:-----:|:----:|:----:|
| 서버 설정(Base URL·버전·IP·자동차단) | O | X | X | X |
| 사용자 관리 | 전체 | user/view만 | X | X |
| 폴더 권한 관리 | O | O | X | X |
| 로그(접근/에러/감사) 조회 | O | O | X | X |
| 업로드·이동·삭제·버전 관리 | 전체 | 전체 | 쓰기 부여 폴더 | X |
| 읽기·다운로드 | 전체 | 전체 | 기본 전체(읽기) | 부여된 폴더만 |
| 미리보기 | O | O | O | X |
| 개인 API 키 발급 | O | O | O | X |
| 서비스 API 키 발급·관리 | O | O | X(목록만·값 마스킹) | X |
| 휴지통·미리보기 | O | O | O | X |
...
curl -H "X-API-Key: <YOUR_API_KEY>" -O -J http://localhost:8180/d/<FILE_ID>

# 업로드 (업로드 스코프 키 필요) — 스크립트 결과 파일 업로드 등
curl -H "X-API-Key: <YOUR_API_KEY>" -F "file=@./result.txt" -F "folder=/results" http://localhost:8180/u

# 헬스체크 (유효한 활성 키일 때만 200, 그 외 404)
curl -H "X-API-Key: <YOUR_API_KEY>" http://localhost:8180/healthz
```

> 스크립트에서 임시로 업로드가 필요하면 **API KEY 탭에서 Upload 스코프 + 만료(예: 1시간/1일) 키**를 발급해 전달하세요. 만료 후 자동으로 거부·정리됩니다.
